// /FireEscapesRoutes
//Problem Link : https://www.codechef.com/LRNDSA08/problems/FIRESC
import java.io.*;
import java.util.*;
public class FireEscapesRoutes{
	static int t,n;
	static int[] arr;
	...
	static boolean[] vis;
	static long dfs(int src,long comp){
		for(Integer nbrs: adj[src]){
			if(!vis[nbrs]){
				vis[nbrs]=true;
				comp++;
				comp=dfs(nbrs,comp);
    	...
    	while(t-->0)
    	{
    		...
    		for(int i=1;i<=n;i++){
    			if(!vis[i]){
    				vis[i]=true;
    				long size=dfs(i,1);
    				ways=((ways%mod)*(size%mod))%mod;
    				comp=(comp+1)%mod;

    			}
    		}
    		sb.append(comp).append(" ").append(ways);
