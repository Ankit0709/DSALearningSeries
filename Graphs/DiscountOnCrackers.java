...
import java.io.*;
import java.util.*;
public class DiscountOnCrackers{
	static int t,n;
	static int[] arr;
    ...
    static boolean[] vis;
    static boolean dfs(char x,char t){
    	if(x==t){
    		return true;
    	}
    	for(Character nbrs:adj[x-'a']){
    		if(!vis[nbrs-'a']){
    			vis[nbrs-'a']=true;
    			if(dfs(nbrs,t)){
    				return true;
    ...
    public static void main(String[] args) throws IOException{
    	...
    	while(t-->0)
    	{
    		...
    		for(int i=0;i<m;i++){
    			String x=s();
    			String str[]=x.split("->");
    			adj[str[0].charAt(0)-'a'].add(str[1].charAt(0));
    		}		
    		boolean flag=true;
